      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-16">
          ...
          <p className="mt-5 text-gray-500 max-w-xl mx-auto leading-relaxed">
            From content creation to scheduling — PostPilot handles it all so
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {features.map((f) => (
            <div
              key={f.title}
              className="bg-white rounded-2xl border border-slate-100 p-6 hover:border-slate-200 hover:shadow-md hover:shadow-slate-100 group"
            >
              <div
                className={`size-10 rounded-xl flex items-center justify-center mb-4 ${f.color}`}
              >
                <f.icon className="size-5" />
              </div>
              <h3 className=" text-slate-900 mb-2">{f.title}</h3>
              <p className="text-sm text-slate-500/90 leading-relaxed">
                {f.description}
              </p>
            </div>
          ))}
